...
const buildTaskTemplate = ({ id, title, createdAt, description, assignedUser, status }) => {
  ...
  return `
    <div class="task p-3 m-1" data-id="${id}">
      ...
      <h4>Assigned user:  ${assignedUser}</h4>
      <div class="task__footer d-flex">
        <select class="task__select" name="status" id="status-select" data-role="status-select" ${selectDisabled}>
        ...
        <div class="task__buttons d-flex mx-3">
          <button type="button" class="task__button task__button_edit btn border-white" id="edit-button"
          ...
          <button type="button" class="task__button task__button_remove btn border-white" data-bs-toggle="modal"
